...
from .fields import ListField, SetField, DictField, EmbeddedModelField, BlobField
...
class BlobFieldModelTestCase(TestCase):

    def test_get_internal_type(self):
        blob = BlobField()
        blob_type = blob.get_internal_type()
        self.assertEquals(blob_type, 'BlobField')

    def test_formfield_kwargs(self, **kwargs):
        blob = BlobField()
        form_field = blob.formfield(max_length=12)
        if hasattr(form_field, 'max_length'):
            pass
        else:
            return error

        if hasattr(form_field, 'min_length'):
            return error
        else:
            pass

    def test_get_db_prep_save_without_file(self, connection=False):
        blob = BlobField()
        value = 1
...
class BlobWidgetTestCase(TestCase):
    ...
    def test_render(self):
        blob_widget = BlobWidget()
        ren = blob_widget.render(name='test', value=1)
        self.assertIn('Current size', ren)
...
class EmbeddedModelFieldTest(TestCase):
    ...
    def _test_pre_save(self, instance, get_field):
        ...
        # auto_now_add has no tzinfo, get_field(instance).auto_now_add does
